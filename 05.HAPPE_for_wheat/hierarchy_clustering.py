...
if __name__ == "__main__":
    ...
    #genetic_distance_matrix = sk.metrics.pairwise.pairwise_distances(pd_df, metric='euclidean')

    # Save the genetic distance matrix to a file
    #np.savetxt("genetic_distance_matrix.csv", dist_matrix, delimiter=",")

    # Save the haplotype network information to a file
    with open("%s.dist.txt"%(sys.argv[3]), "w") as haplotype_network_file:
        haplotype_network_file.write("Sample1,Sample2,Distance,Cluster1,Cluster2\n")
        for i, sample1 in enumerate(col_name):
            for j, sample2 in enumerate(col_name):
                if i < j:
                    haplotype_network_file.write(f"{sample1},{sample2},{dist_matrix[i, j]},{clusters['labels'][i]},{clusters['labels'][j]}\n")
