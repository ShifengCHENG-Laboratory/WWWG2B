    ...
    for anno_block in inf_line:
        if re.match('ANN', anno_block):  # 从头匹配ANN，详细注释从这里开始
            ...
            for anno_inf in ANN_block:
                ...
                cxx = anno_inf[9]
                pxx = anno_inf[10]
                if feature_id not in anno_dic: # 为该行snp创造一个全新的字典存储信息
                    anno_dic[feature_id] = []
...
# 此后几个判断是精简加入的注释信息，以防其影响后续排序。
                if annotation == 'intergenic_region':
                    ...
                    anno_dic[feature_id][3].add(impact)
    ...
    for id in anno_dic:  # 将每个feature_id对应的所有注释转化为1个
        ...
        impact_list = anno_dic[id][3]
        all_impact = '|'.join(impact_list)
        final_all_anno.append([id, all_anno, all_cxx, all_pxx, all_impact])
    ...
    snp_ann_df = pd.DataFrame(
        final_all_anno, columns=['Feature_id', 'Annotation', 'c.xx', 'p.xx','impact'])
    ...
    out_anno = (sort_ann_df.loc[0, 'Feature_id'],
                ...
                sort_ann_df.loc[0, 'c.xx'], sort_ann_df.loc[0, 'p.xx'],sort_ann_df.loc[0,'impact'])
