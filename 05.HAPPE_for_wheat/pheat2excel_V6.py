...
# add excel cell annotation,will make the excel slow
    ...
    while line:
        if not line.startswith("#"):
            ...
            if ls[2] in snp_ann:
                ...
                snp_ann[ls[2]][1] = ls[4]
                feature_id , annotation , cHGVS , pHGVS =  SNPANN_upgrade_3.SNP_ANN_of_Gene_Structure(ls[7])
        ...
        for index1,value in enumerate(sample_gt[sample]):
            ...
            if cell_row > 1 and first_row and False:
                ...
                for i in range(cell_row):
                    ...
                    color_fillgt = PatternFill(fgColor=color_gt, fill_type="solid")
                    #### comment
                    snp_id = snp_list[index1]
                    snpi =  snp_ann[snp_id]

                    ### comment
                    comment = Comment("sample:%s\n%s\n%s"%(sample , snp_id , snpi[2]), #sample, snpid , feature id
                        "fengcong@caas.cn",width=200, height=70)
                    ws.cell(row=merge_range_item1[0]+i,column=merge_range_item1[1]).comment = comment
