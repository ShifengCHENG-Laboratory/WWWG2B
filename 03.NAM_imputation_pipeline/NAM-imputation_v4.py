...
def thread_func(job_queue,axiom_data_dict,output_file_dict,lock,thread_id,threads_id_list,work_status):
    #job queue
    ...
    for indx in output_file_dict:
        output_writer_dict[indx] =  write_vcf_header(output_file_dict[indx][0], list(axiom_data_dict[output_file_dict[indx][1]].keys()) )
...
def impute(axiom_data_dict, vcf_file_or_list,output_prefix,threads):
    ...
    for file_reader in file_reader_list:
        ...
        if len(watseq_index) == 0:
            for watkins_parents in axiom_data_dict.keys():
                if watkins_parents in header_list:
                    ...
                    output_file_dict[header_list.index(watkins_parents)] = \
                        (output_prefix+".Paragonx%s.vcf.gz"%(watkins_parents),watkins_parents)
            ...
            for chunk in watseq_index_chunk_list:
                ...
                threads_list.append(Process(target=thread_func,args=(tmppq,axiom_data_dict,tmp_output_file_dict,lock,thread_id,[id for id in range(0,used_threads_num)],work_status)))
                work_status[thread_id] = 0
...
def read_block_data(block_file,axiom_data_dict):
    ...
    with open(block_file,"r") as block_f:
        ...
        while line:
            ...
            for watkins_parent in axiom_data_dict:
                for recombinant in axiom_data_dict[watkins_parent]:
                    if block_chr in axiom_data_dict[watkins_parent][recombinant]:
                        tmp_d = axiom_data_dict[watkins_parent][recombinant][block_chr]
                        
                        idx_start = np.searchsorted(tmp_d["position"], block_start, side="left")
                        idx_end = np.searchsorted(tmp_d["position"], block_end, side="right")

                        tmp_d["position"] = tmp_d["position"][idx_start:idx_end]
                        tmp_d["genotype"] = tmp_d["genotype"][idx_start:idx_end]
                        tmp_d["id"] = tmp_d["id"][idx_start:idx_end]
                        tmp_d["status"] = -1 # must be 0(same GT in this block) or 1(diff GT in this block) or 2(no GT in this block)

                        ##calc status
                        if len(tmp_d["genotype"]) == 0:
                            tmp_d["status"] = 2
                        elif len(set(tmp_d["genotype"])) == 1:
                            tmp_d["status"] = 0
                        else:
                            tmp_d["status"] = 1
                        ...
                        ##calc status
                        block_status_dict[block_id] \
                        .setdefault(watkins_parent,{})[recombinant] = tmp_d
                    else:
                        tmp_d = {}
                        tmp_d["status"] = 2
    ...
    return(block_boundaries_list_dict,block_status_dict)
...
if __name__ == "__main__":
    ...
    print("axiom_data_dict done",datetime.datetime.now())
    block_boundaries_list_dict,block_status_dict = read_block_data(block_file,axiom_data_dict)
    print("block_status_dict done",datetime.datetime.now())
    with open("block_status_dict","w+") as jsf:
        jsf.write(json.dumps(block_status_dict,indent=4))
    with open("block_boundaries_list_dict","w+") as jsf:
        jsf.write(json.dumps(block_boundaries_list_dict,indent=4))
    exit()

    impute(axiom_data_dict, vcf_file_or_list,output_prefix,threads)
